...
# -------------------------------------------------------------
st.set_page_config(page_title="Painel Visual Projeto Phoenix", layout="wide", page_icon="⭐")
...
# ============================================================
# POPUP FÊNIX — Versão Premium com Cards + Blur + Gradiente
# ============================================================

import streamlit as st
import datetime

if "popup_show" not in st.session_state:
    st.session_state.popup_show = True
...
def popup_fenix():
    ...
    #popup-box {
        width: 520px;
        max-width: 92vw; /* 🔥 evita estourar no mobile */
        padding: 38px 32px;
        border-radius: 22px;
        background: rgba(15,15,15,0.85);
        border: 1px solid rgba(255,255,255,0.15);
        box-shadow: 0 0 40px rgba(0,255,180,0.22);
        text-align: center;
        color: white;
    ...
    .btn-oferta {
        ...
        width: 90%;
        transition: 0.3s;
        box-shadow: 0 0 18px rgba(255,100,0,0.35);
    }

    .btn-oferta:hover {
        transform: scale(1.05);
        box-shadow: 0 0 25px rgba(255,100,0,0.55);
    }


    /* =======================================================
       🔥 RESPONSIVIDADE — MODO MOBILE (iPhone / Android)
       ======================================================= */
    ...
    <div id="popup-overlay">
        <div id="popup-box">
            ...
            <div class="titulo">Até 80% OFF</div>
            <div class="subtitulo">+ 1 mês gratuito</div>

            <div class="texto-menor">A oferta termina em</div>

            <!-- CONTADOR COM CARDS -->
            <div class="timer-container">
                <div class="timer-card">
                    <div id="dias" class="timer-number">00</div>
                    <div class="timer-label">DIAS</div>
                </div>
                <div class="timer-card">
                    <div id="horas" class="timer-number">00</div>
                    <div class="timer-label">HORAS</div>
                </div>
                <div class="timer-card">
                    <div id="minutos" class="timer-number">00</div>
                    <div class="timer-label">MINS</div>
                </div>
                <div class="timer-card">
                    <div id="segundos" class="timer-number">00</div>
                    <div class="timer-label">SEGS</div>
                </div>
            </div>

            <button class="btn-oferta" id="btn-acesso">
                Confira as ofertas
            </button>
    ...
    function updateTimer() {
        ...
        document.getElementById("dias").textContent = ("0" + dias).slice(-2);
        document.getElementById("horas").textContent = ("0" + horas).slice(-2);
    ...
    // --- LINK FUNCIONAL (em nova aba) ---
    document.getElementById("btn-acesso").onclick = function() {
        window.open("https://loja.infinitepay.io/aurinvest/uay5017-carteira-ibov", "_blank");
    };
...
if st.session_state.popup_show:
    popup_fenix()
...
if secao == "Painel":
    ...
    # =====================================================================
    aba_total, aba_ibov, aba_smll, aba_bdr = st.tabs(
        ["📊 Total", "🟦 IBOV", "🟩 SMLL", "🟨 BDR"]
    )

    aba_atual = "TOTAL"

    if st.session_state.get("active_tab") != None:
        aba_atual = st.session_state["active_tab"]


    # =====================================================================
    # ⭐ ABA TOTAL – PAINEL COMO ESTAVA ANTES
    # =====================================================================
    with aba_total:
        st.session_state["active_tab"] = "TOTAL"
    
        # 1) TRADES PENDENTES — CARD DO ROBÔ CURTO
        render_robot_card(ROBOS[0], st.container())
    
        # 2) MONITORAMENTO — TRADES PENDENTES (todos os ativos)
        render_pendentes_cards(curto_state.ativos)
    
        st.markdown("---")
    
        # 3) TRADES EM ANDAMENTO — CARD DO ROBÔ LOSS CURTO
        render_robot_card(ROBOS[1], st.container())
    
        # 4) MONITORAMENTO — LOSS CURTO (todos os ativos)
        if not loss_state.ativos:
            st.info("Nenhum ativo configurado no robô LOSS CURTO.")
        else:
            render_andamento_cards(loss_state.ativos)
    
        # ---------------------------------------------
        # 🔥 RESUMO 30 DIAS — TOTAL
        # ---------------------------------------------
        render_resumo_30d("TOTAL")
    
    
    
    # =====================================================================
    # ⭐ ABA IBOV – APENAS ATIVOS COM INDICE == 'IBOV'
    # =====================================================================
    with aba_ibov:
        st.session_state["active_tab"] = "IBOV"
    ...
    # =====================================================================
    # ⭐ ABA SMLL – APENAS ATIVOS COM INDICE == 'SMLL'
    # =====================================================================
    with aba_smll:
    ...
    # =====================================================================
    # ⭐ ABA BDR – APENAS ATIVOS COM INDICE == 'BDR'
    # =====================================================================
    with aba_bdr:
        st.session_state["active_tab"] = "BDR"
